...
export default function ProfileClient({ params }: ProfilePageProps) {
  ...
  const [nftCollectionsCollapsed, setNftCollectionsCollapsed] = useState(false)
            ...
            <div className="flex flex-col md:flex-row items-start gap-4 md:gap-6">
              ...
              <div className="flex-1">
                ...
                {profile.profile?.isProject && profile.profile?.profileVerified && (
                  <div className="mb-4">
                    <div className="inline-flex items-center gap-2 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 px-4 py-2 rounded-xl">
                      <span className="text-2xl">🏢</span>
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-cyan-300 text-sm font-semibold">Verified Project</span>
                          {profile.profile.projectType && (
                            <span className="bg-blue-500/30 text-blue-200 text-xs px-2 py-0.5 rounded-full font-medium">
                              {profile.profile.projectType}
                            </span>
                          )}
                        </div>
                        {(profile.profile.projectWebsite || profile.profile.projectTwitter || profile.profile.projectDiscord) && (
                          <div className="flex items-center gap-2 mt-1">
                            {profile.profile.projectWebsite && (
                              <a
                                href={profile.profile.projectWebsite}
                                target="_blank"
                            ...
                            {profile.profile.projectTwitter && (
                              <a
                                href={profile.profile.projectTwitter}
                                target="_blank"
                                rel="noopener noreferrer"
                            ...
                            {profile.profile.projectDiscord && (
                              <a
                                href={profile.profile.projectDiscord}
                                target="_blank"
                                rel="noopener noreferrer"
                    ...
                    <div key={share.id} className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
                      ...
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
                        <div className="flex items-center gap-6">
                          <span className="text-gray-300">❤️ {share.post.stats.likes}</span>
                          <span className="text-gray-300">💬 {share.post.stats.comments}</span>
                          <span className="text-gray-300">🔄 {share.post.stats.shares}</span>
                        </div>
                      </div>
                    </div>
                  ))
